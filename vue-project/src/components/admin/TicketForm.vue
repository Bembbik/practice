  ...
  computed: {
    ...
    availableTeams() {
      ...
      const selectedDepartment = this.departments.find(
        (dept) => dept.departmentId === this.formData.departmentId,
      )
      return selectedDepartment ? selectedDepartment.teams || [] : []
    },
  },
  watch: {
    ticket: {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        // Удаляем id при создании нового тикета
        if (!this.editing) {
        ...
        if (this.editing) {
          // При редактировании отправляем PUT-запрос с id тикета
          await axios.put(`${apiUrl}/admin/tickets/${submitData.id}`, submitData, {
            headers: {
              'Content-Type': 'application/json',
            },
          })
        } else {
          // При создании отправляем POST-запрос без id
          await axios.post(`${apiUrl}/admin/tickets`, submitData, {
            headers: {
              'Content-Type': 'application/json',
            },
          })
